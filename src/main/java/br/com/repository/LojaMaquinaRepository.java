...
public interface LojaMaquinaRepository extends JpaRepository<LojaMaquina, Long> {
	...
	@Query(value = "SELECT new br.com.service.dto.LojaMaquinaDTO(lm.id, m.id, m.nome, l.id, l.nomeFantasia) FROM LojaMaquina lm JOIN Maquina m ON m.id = lm.maquina.id JOIN Loja l ON l.id = lm.loja.id")
