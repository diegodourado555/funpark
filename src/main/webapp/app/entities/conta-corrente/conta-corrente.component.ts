...
export class ContaCorrenteComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.contaCorrenteService
      ...
      .subscribe((res: HttpResponse<IContaCorrente[]>) => this.paginateContaCorrentes(res.body, res.headers));
  }
  ...
  trackId(index: number, item: IContaCorrente) {
    return item.id;
  }

  registerChangeInContaCorrentes() {
    this.eventSubscriber = this.eventManager.subscribe('contaCorrenteListModification', () => this.loadAll());
  }

  delete(contaCorrente: IContaCorrente) {
  ...
  protected paginateContaCorrentes(data: IContaCorrente[], headers: HttpHeaders) {
    this.links = this.parseLinks.parse(headers.get('link'));
    this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
    this.contaCorrentes = data;
  }
}
